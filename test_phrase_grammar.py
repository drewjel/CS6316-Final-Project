...
criterion = nn.sparse_softmax_cross_entropy_with_logits()
...
def test(model, corpus, cuda, prt=False):
    ...
    for sen, sen_tree, sen_nltktree in dataset:
        ...
        input = tf.Variable(tf.Tensor(x[:, None]))
        if cuda:
            input = input.cuda()
        ...
        if prt and nsens % 100 == 0:
            ...
            _, axarr = plt.subplots(3, sharex=True, figsize=(distance.shape[1] // 2, 6))
